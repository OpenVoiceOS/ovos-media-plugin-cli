"""Unit tests for the Simple backends.
...
from ovos_media_plugin_simple import SimpleAudioService, SimpleBaseService
from ovos_media_plugin_simple.audio import SimpleOldAudioService, load_service


class TestNewBackend(unittest.TestCase):
    def test_audio_backend_is_audioplayerbackend(self):
        svc = SimpleAudioService({}, bus=MagicMock())
        self.assertIsInstance(svc, AudioPlayerBackend)
        self.assertIsInstance(svc, SimpleBaseService)
...
class TestLegacyAdapter(unittest.TestCase):
    def test_legacy_is_audiobackend(self):
        svc = SimpleOldAudioService({}, bus=MagicMock(), name='simple')
    ...
    def test_load_service_builds_active_simple_backends(self):
        cfg = {"backends": {
            "mysimple": {"type": "simple", "active": True},
            "off": {"type": "simple", "active": False},
            "other": {"type": "vlc", "active": True},
        }}
        services = load_service(cfg, bus=MagicMock())
        self.assertEqual(len(services), 1)
        self.assertIsInstance(services[0], SimpleOldAudioService)
...
class TestEntryPoints(unittest.TestCase):
    ...
    def test_setup_declares_new_and_legacy_groups(self):
        ...
        self.assertIn("ovos_media_plugin_simple.audio", setup_src)
